...
def main(argv):
    ...
    if 'finetune' in FLAGS.train_mode or (FLAGS.train_suphead_while_pretraining and FLAGS.pretrain_on_train_only):
        ...
        global_loss_metric = tf.keras.metrics.Mean('train/global_total_loss')
        ...
        global_acc_metric = tf.keras.metrics.Mean('train/global_acc')
        supervised_msens_metric = metrics.MeanSensitivity('train/supervised_msens')
        global_msens_metric = tf.keras.metrics.Mean('train/global_msens')
        ...
        global_metrics = [global_loss_metric,global_acc_metric,global_msens_metric]
        all_metrics.extend([supervised_total_loss_metric,supervised_loss_metric, supervised_acc_metric,supervised_msens_metric])
    ...
    if 'finetune' in FLAGS.train_mode:
        ...
        ds_tr_f, ds_val_f = ds_builder_finetune.build_dataset(FLAGS.train_batch_size, dist, is_training = True, seed=FLAGS.seed)
        
        num_tr_data  = ds_builder_finetune.num_train_examples
        num_val_data = ds_builder_finetune.num_eval_examples
        
        
        f_distrib_lr =  FLAGS.learning_rate_finetuning * dist.size()
        f_learning_rate = model_lib.step_decay(f_distrib_lr,
                                               num_tr_data)
...
#                f_model, f_optimizer.iterations, f_optimizer)
        ...
        def finetune_single_step(features, labels):
            ...
            #gradient() cannot be invoked within the GradientTape context (i.e., while operations are being recorded)
            ...
            if f_optimizer.iterations == 0:
               # Broadcast model and optimizer variables
                dist.broadcast_variables(f_model.variables, root_rank=0)
                dist.broadcast_variables(f_optimizer.variables(), root_rank=0)
        ...
        while cur_fine_step < train_steps_fine:
            ...
            # Computing average metrics across all workers
            metrics.allreduce_global_metrics(dist,global_loss_metric, global_acc_metric, global_msens_metric,
                                    supervised_total_loss_metric, supervised_acc_metric,supervised_msens_metric)
